...
class PrivateContestCancel implements ShouldQueue
{
    ...
    {
        $contest = PrivateContest::query()->find($this->contestId);
        ...
        if ($contest && ( $contest->status == CONTEST_STATUS[1] || $contest->status == CONTEST_STATUS[2] ) ) {
            ...
            DB::transaction(function () use ($contest, $joined) {
                ...
                Redis::srem("private_contest_in_cancelling", $this->contestId);
            });
        }
    }
}
